...
// GET data idPeminjamFiksi
$idPeminjamFiksi = $_GET['idPeminjamFiksi'];

// Query
$pminjaman = query("SELECT * FROM peminjamanbukufiksi WHERE idPeminjamFiksi = $idPeminjamFiksi")[0];

// Cek Submit
if (isset($_POST['submit'])) {

	// Cek apakah data berhasil di Edit atau Tidak
	if (editPeminjamanFiksi($_POST) > 0) {
...
<!--form input data Buku Peminjaman-->
	...
	<div class="card shadow-lg border-top-0 rounded" style="width: 80%; height: 65vh;"><!--card-->
		<form  action="" method="POST">
			<input type="hidden" name="idPeminjamFiksi" id="idPeminjamFiksi" value="<?= $pminjaman["idPeminjamFiksi"]; ?>"> <!--Menyembunyika Input Id-->
			<div class="d-flex p-2" style="justify-content: center;"><!--top-->
				...
				<div class="mb-4 mt-5" style="width: 40%; margin-left: 10%;">
					...
					<select name="idBukuFiksi" id="idBukuFiksi" class="form-control">
			...
			<div class="d-flex p-2 " style="justify-content: center;"><!--bottom-->
				<div class="mb-4 mt-5" style="width: 40%; margin-left: 10%;">
					...
					<input type="date" name="tglPinjam" id="tglPinjam" value="<?= $pminjaman["tglPinjam"]; ?>" class="form-control">
				</div>
			</div>
